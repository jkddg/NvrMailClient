...
@Slf4j
@Component
public class AlarmCaptureService {

    private Map<Integer, LocalDateTime> alarmTimeMap = new HashMap<>();//key=通道号，value=上次预警时间
...
//                            log.info("通道名：" + channelInfo.getName() + "，通道号：" + channel + "预警间隔不够" + NvrConfigConstant.alarmIntervalSecond + "秒，丢弃");
        ...
        if (streamFile != null) {
            capturePool.alarmPush(channelInfo, streamFile);
        } else {
